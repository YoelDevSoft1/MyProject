...
const GoogleAuthAlternative = ({ onSuccess, onError, isLoading, disabled }) => {
    ...
    script.onload = () => {
      if (window.google && window.google.accounts && window.google.accounts.id) {
        try {
          ...
          console.log('Google Auth initialized successfully (alternative method)');
        } catch (error) {
  ...
  return (
    ...
    >
      <HStack spacing={3}>
        ...
        <Text>
          {isGoogleReady ? "Continuar con Google (Alternativo)" : "Google Auth no disponible"}
        </Text>
      </HStack>
    </Button>
  );
...
export default GoogleAuthAlternative;
