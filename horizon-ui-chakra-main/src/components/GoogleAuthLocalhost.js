...
const GoogleAuthLocalhost = ({ onSuccess, onError, isLoading, disabled }) => {
  ...
  const handleCredentialResponse = useCallback((response) => {
    try {
      console.log('Google credential response received (localhost)');
      ...
      console.log('Google user data (localhost):', userData);
      onSuccess(userData);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Forzar el uso de localhost en lugar de 127.0.0.1
    if (window.location.hostname === '127.0.0.1') {
      console.log('Redirecting from 127.0.0.1 to localhost for Google OAuth compatibility');
    ...
    script.onload = () => {
      if (window.google && window.google.accounts && window.google.accounts.id) {
        try {
          ...
          console.log('Google Auth initialized successfully (localhost method)');
        } catch (error) {
  ...
  if (isLoadingGoogle) {
    return (
      ...
      >
        <HStack spacing={3}>
          <Spinner size="sm" />
          <Text>Cargando Google Auth (localhost)...</Text>
        </HStack>
      </Button>
    );
  }
  ...
  return (
    ...
    >
      <HStack spacing={3}>
        ...
        <Text>
          {isGoogleReady ? "Continuar con Google (localhost)" : "Google Auth no disponible"}
        </Text>
      </HStack>
    </Button>
  );
...
export default GoogleAuthLocalhost;
