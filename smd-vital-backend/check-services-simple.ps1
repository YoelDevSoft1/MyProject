# Script de Verificacion de Servicios SMD VITAL
# =============================================
...
function Test-Service {
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            Write-Host "OK $Name (Puerto $Port): SALUDABLE" -ForegroundColor Green
            return $true
        } else {
            Write-Host "ERROR $Name (Puerto $Port): Status $($response.StatusCode)" -ForegroundColor Red
            return $false
        }
    } catch {
        Write-Host "ERROR $Name (Puerto $Port): NO DISPONIBLE" -ForegroundColor Red
        return $false
    }
}

# Verificar servicios principales
Write-Host "SERVICIOS DE AUTENTICACION:" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "SERVICIOS MEDICOS:" -ForegroundColor Yellow
